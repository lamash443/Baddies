    ...
    @if($activeUser || $activeUserId === 'announcement')
        ...
        <div id="chat-header-row" class="chat-header-row position-relative flex-shrink-0" style="height: 62px; min-height: 62px; max-height: 62px; overflow: hidden; border-bottom: 1px solid rgba(255,140,0,0.25); background-color: var(--header-bg); background-image: linear-gradient(135deg, rgba(255,140,0,0.15) 0%, rgba(255,140,0,0.05) 100%);">
            ...
            <div id="chat-std-header" :class="selectedIds.length === 0 ? 'd-flex' : 'd-none'" class="d-flex align-items-center px-3 py-2 h-100 w-100">
                ...
                {{-- Back button --}}
                <a href="{{ route('chat.index') }}" wire:navigate class="text-decoration-none me-2 d-flex align-items-center justify-content-center flex-shrink-0" title="Back"
                    ...
                    @else
                        ...
                        @if($showStatusInChat)
                            ...
                            @elseif($activeUser->last_seen_at)
                                <small style="color: var(--header-subtext); font-size: 0.7rem;">Last seen {{ $activeUser->last_seen_at->diffForHumans() }}</small>
                            @else
    ...
    @else
        <div class="d-flex flex-column h-100 justify-content-center align-items-center text-center p-4 position-relative" style="background: linear-gradient(135deg, rgba(255,140,0,0.05) 0%, rgba(0,0,0,0) 100%);">
            ...
            <div class="position-relative" style="z-index: 10;">
                ...
                @endphp
                @if($siteLogo)
                    <img src="{{ asset('storage/'.$siteLogo) }}" alt="Kenyan Baddies" class="mb-4" style="max-height: 80px; opacity: 0.9; filter: drop-shadow(0 0 15px rgba(255,140,0,0.25));">
                @else
